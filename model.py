...
class YOLOXNeck(nn.Module):
    def __init__(self):
        ...
        self.downsample_2=nn.Conv2d(in_channels=256,
            out_channels=256,
            kernel_size=3,
            stride=2,
            padding=1
            )
        self.down_block_2 = CSPBlock(in_channels=512,
         out_channels=256,
          num_blocks=5,
           shortcut=False
           )
    ...
    def forward(self, backbone_outputs):
        shallow, mid, deep = backbone_outputs #[1, 256, 80, 80],[1, 256, 40, 40],[1, 256, 20, 20]
        ...
        mid_processed_upwnsampled=F.interpolate(mid_processed,scale_factor=2,mode="nearest")
        ...
        shallow_downsamplled_concated = torch.cat((shallow_downsamplled, mid_processed), dim=1)
